...
def consistent(assignment, csp, var, value):
	for constraint in csp.binaryConstraints:
		if constraint.affects(var):	
			...
			if assignment.assignedValues.get(otherVariable) != None:
				if not constraint.isSatisfied(value,assignment.assignedValues.get(otherVariable)):
					return False
	return True
...
def recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod):
	...
	for value in orderValues(assignment,csp,var):
		if consistent(assignment,csp,var,value):
			...
			inf = inferenceMethod(assignment,csp,var,value)
...
def minimumRemainingValuesHeuristic(assignment, csp):
	...
	for key in assignment.assignedValues:
		if assignment.assignedValues[key] == None:
			constraintsSize[key] = len(domains[key])
	sorted_x = sorted(constraintsSize.items(), key=lambda constraintSize: constraintSize[1])
	print sorted_x
	for item in sorted_x:
		if item[1] == sorted_x[0][1]:
			tieBreak[item[0]] = 0
	for item in sorted_x:
		if item[1] == sorted_x[0][1]:
			for constraint in csp.binaryConstraints:
				if constraint.affects(item[0]):
					tieBreak[item[0]] += 1	
	sorted_y = sorted(tieBreak.items(), key=lambda x: x[1])
	return sorted_y[len(sorted_y)-1][0]
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
	...
	orderedVals = []
	for val in values:
		listX.append((val,countConstraints(assignment, csp, var, val)))
	sorted_y = sorted(listX, key=lambda x: x[1][0])
	
	#print values
	#for constraint in csp.binaryConstraints:
	#	print constraint
	#	for item in (constraint.var1,constraint.var2):
	#		tieBreak[item]+=1
	#sorted_x = sorted(tieBreak.items(), key=lambda constraintSize: constraintSize[1])
	"""Hint: Creating a helper function to count the number of constrained values might be useful"""
	"""Question 3"""
	"""YOUR CODE HERE"""
	for item in sorted_y:
		orderedVals.append(item[0])
	return orderedVals[::-1]
def countConstraints(assignment,csp, var, val):
	count = 0;
	myDomains = assignment.varDomains
	domains = copy.deepcopy(myDomains)
	if var in domains:
		del domains[var]
	for constraint in csp.binaryConstraints:
